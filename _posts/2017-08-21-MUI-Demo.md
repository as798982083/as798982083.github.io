...
{% highlight javascript %}
<script type="text/javascript">
	mui.ajax('https://www.baidu.com',{
		async:true, //发送同步请求
		crossDomain:true, //强制使用5+跨域
		data:{
			dw:'谷歌'
		},
		dataType:'json', //服务器返回json格式数据,还支持xml、html、script、text。
		type:'get', //HTTP请求类型
		timeout:10000, //超时时间为10秒。默认值为0，即永不超时。超时后触发error回调
		headers:{'Content-Type':'application/json'},//json类型
		success:function(data,textStatus,xhr){
			//data:服务器返回的相应数据，类型可以是json、xml对象，字符串等
			//textStatus:状态描述，默认值为：success
			//xhr:实例对象
			//服务器返回相应，根据相应结果，分析是否登录成功；
			//...
		},
		error:function (xhr,type,errorThrown) {
			//xhr:XMLHttpRequest 实例对象
			//type:错误描述，可取值：timeout,error,abort,parserror,null
			//errorThrown:String,可捕获的一场对象
			//异常处理；
			console.log(type); 
		},
		processData:true
		...
		//如果业务需要，希望发送其它格式的数据（比如Document对象），可以设processData为false；
	});
</script>
{% endhighlight %}

**注**：`mui.get()`和`mui.post()`均不处理timeout和异常。
...
单webview模式示例图：  
![单webview模式](/img/mui/siglefresh.gif)  

动画原理：  
刷新时触发的是原生下拉刷新控件，webview位置不发生改变，也不会出现DOM重绘。性能比双webview更优。

使用方法：  
1. mui初始化时设置pullRefresh各项参数，与双webview模式的子页面设置是一样的。
2. DOM结构无特殊要求，只需要指定一个下拉刷新容器即可。
...
**代码解释：**  
1. `pullRefresh()`：函数调用原生的下拉刷新控件。
2. 目前仅支持`cricle`样式及该样式的颜色自定义。

## 双webview模式

双webview模式示例图：  
![双webview模式](/img/mui/duoblefresh.gif)  

动画原理：  
拖动时，拖动的是整个webview，回弹动画使用的是原生动画；
...
内容页面需要统一DOM结构，以保证Android和IOS连个平台DOM结构一致，如下：
...
并在内容页面通过`mui.init`方法中的pullRefresh参数配置下拉刷新各项参数，如下：  
	...
	mui.init({
		pullRefresh: {
			...
			down: {
				...
				//height:'10px',	//可选,默认50px.下拉刷新控件的高度,（经测试，下拉刷新时，不可用）
		        range:'100px', 	//可选 默认100px,控件可下拉拖拽的范围
		        offset:'0px', 	//可选 默认0px,下拉刷新控件的起始位置
		        auto: false, 	//可选,默认false.首次加载自动上拉刷新一次
				contentrefresh: '下拉刷新正在加载...',
				callback: pulldownRefresh
...
`scrollTo()`函数目前仅支持双webview模式。

## 自动触发下拉刷新

首次加载时自动触发一次上或下拉刷新，配置参数`auto:true`即可：
...
在下拉刷新的触发函数中执行`endPulldown`方法，来关闭"正在刷新"的样式提示，内容区域回滚至顶部，如下：  
...
参数Type: Boolean，是否还有更多数据；若还有更多数据，则传入false; 否则传入true，之后滚动条滚动到底时，将不再显示“上拉显示更多”的提示语，而显示“没有更多数据了”的提示语；
...
在使用过`.endPullupToRefresh(true)`之后，可通过调用.refresh(true)方法，可重置上拉加载控件。如下：  
...
若不希望显示“上拉显示更多”之类的标语，可通过调用`disablePullupToRefresh()`禁止上拉刷新。如下：  
