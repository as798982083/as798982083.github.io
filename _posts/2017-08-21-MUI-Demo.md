...
`mui-draggable`: 默认侧滑样式定义，主界右移，菜单不动。 
...
> 在`mui`中，Picker有两种具体表现：`.poppicker`和`.dtpicker`

1. Poppicker：   
特点：支持多级联动。  

示例图：  
![选择器](/img/mui/picker.gif)  
...
**代码解释：**  
`mui.picker.js/.css mui.poppicker.js/.css`：.poppicker的依赖组件，务必在`mui.js/css`后引用,也可统一引用压缩版本`mui.picker.min.js`。  
`new PopPicker()`: 初始化poppicker组件。  
`setData()`： 添加数据，格式为数组。  
